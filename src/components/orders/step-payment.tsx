    ...
    <div className="space-y-8 animate-in slide-in-from-right">
        ...
        <table className="w-full text-left border-collapse">
          <thead className="bg-slate-100 text-[10px] font-black uppercase text-slate-500 italic">
            <tr>
              ...
              <th className="p-4 w-36 text-center border-b-2 bg-brand-50 text-brand-800">Paciente</th>
              ...
              return (
                <tr key={index} className="hover:bg-slate-50 transition-colors">
                  ...
                  <td className="p-4 bg-brand-50/30">
                    <div className="relative flex items-center justify-center">
                      <span className="absolute left-3 text-brand-700 text-sm">$</span>
                      <Input type="number" className="pl-6 h-9 w-full text-center font-black text-brand-700 border-2 border-brand-200 focus-visible:ring-brand-700 bg-white" value={item.patientCopay || 0} onChange={(e) => onUpdateItemPrice(index, 'patientCopay', Number(e.target.value))}/>
                    </div>
                  </td>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="flex flex-col gap-4">
          ...
          <div className="bg-slate-900 p-5 rounded-2xl border-t-8 border-brand-700 flex flex-col justify-center shadow-xl text-center">
            <p className="text-[11px] font-black text-brand-400 uppercase tracking-widest mb-1">{editingOrderId ? "DIFERENCIA A COBRAR" : "A COBRAR AL PACIENTE"}</p>
        ...
        <div className="md:col-span-2 bg-white border-2 border-slate-200 rounded-2xl p-6 shadow-sm">
          <div className="flex justify-between items-center mb-4 border-b pb-3">
            <h4 className="font-black italic uppercase text-slate-800 flex items-center gap-2"><CreditCard size={18} className="text-brand-700"/> Medios de Pago</h4>
            {targetAmount > 0 && (
              ...
              {form.watch("paymentsList").map((payment: any, index: number) => (
                <div key={index} className="flex items-center gap-3">
                  ...
                  {form.watch("paymentsList").length > 1 && (
                    <Button variant="ghost" onClick={() => onRemovePayment(index)} className="text-slate-400 hover:text-brand-600 shrink-0"><Trash2 size={18}/></Button>
                  )}
                </div>
              ))}
              <div className={`mt-4 p-3 rounded-xl flex justify-between items-center font-black uppercase italic transition-colors ${remainingBalance === 0 ? 'bg-emerald-100 text-emerald-800 border border-emerald-200' : 'bg-brand-50 text-brand-700 border border-brand-200'}`}>
