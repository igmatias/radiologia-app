...
function QuickDentistForm({ onSuccess }: { onSuccess: () => void }) {
  ...
  const handleSubmit = async () => {
    ...
    setLoading(true);
    const res = await importDentistsAction([{ firstName: data.Nombre, lastName: data.Apellido, matriculaProv: data.MatriculaProvincial, email: data.Email, deliveryMethod: data.deliveryMethod, isActive: true }]);
