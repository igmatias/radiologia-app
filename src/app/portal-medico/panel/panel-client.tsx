  ...
  return (
    <>
      ...
      <Dialog open={showGuide} onOpenChange={setShowGuide}>
        <DialogContent className="sm:max-w-sm bg-neutral-900 border border-neutral-700 rounded-2xl p-6 outline-none">
          <DialogHeader>
            <DialogTitle className="text-white font-black uppercase text-lg">Instalar como App</DialogTitle>
          </DialogHeader>
          <ol className="space-y-3 text-neutral-300 text-sm mt-2">
            {isIOS ? (<>
              <li className="flex items-start gap-2"><span className="bg-[#BA2C66] text-white rounded-full w-5 h-5 flex items-center justify-center text-xs font-black shrink-0 mt-0.5">1</span>Tocá el botón <strong className="text-white">Compartir</strong> (cuadrado con flecha) en Safari</li>
              <li className="flex items-start gap-2"><span className="bg-[#BA2C66] text-white rounded-full w-5 h-5 flex items-center justify-center text-xs font-black shrink-0 mt-0.5">2</span>Desplazá y tocá <strong className="text-white">"Agregar a pantalla de inicio"</strong></li>
              <li className="flex items-start gap-2"><span className="bg-[#BA2C66] text-white rounded-full w-5 h-5 flex items-center justify-center text-xs font-black shrink-0 mt-0.5">3</span>Tocá <strong className="text-white">Agregar</strong> en la esquina superior</li>
            </>) : (<>
              <li className="flex items-start gap-2"><span className="bg-[#BA2C66] text-white rounded-full w-5 h-5 flex items-center justify-center text-xs font-black shrink-0 mt-0.5">1</span>Tocá el menú <strong className="text-white">⋮</strong> (tres puntos) en Chrome</li>
              <li className="flex items-start gap-2"><span className="bg-[#BA2C66] text-white rounded-full w-5 h-5 flex items-center justify-center text-xs font-black shrink-0 mt-0.5">2</span>Tocá <strong className="text-white">"Agregar a pantalla de inicio"</strong></li>
              <li className="flex items-start gap-2"><span className="bg-[#BA2C66] text-white rounded-full w-5 h-5 flex items-center justify-center text-xs font-black shrink-0 mt-0.5">3</span>Confirmá tocando <strong className="text-white">Instalar</strong></li>
            </>)}
          </ol>
          <button onClick={() => setShowGuide(false)} className="w-full py-3 bg-neutral-800 text-neutral-300 rounded-xl text-sm font-bold uppercase mt-4">Cerrar</button>
        </DialogContent>
      </Dialog>
    </>
  )
        ...
        <div className="space-y-6 mt-8">
          {dentist.orders?.length === 0 ? (
            ...
            filteredOrders.map((order: any) => {
              ...
              const statusLabel = order.status === 'ENTREGADA' ? 'Entregado' : isListo ? '✅ Listo' : order.status === 'LISTA' ? 'Listo' : 'En proceso'
