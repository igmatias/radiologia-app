...
import { createDerivacion } from "@/actions/derivaciones"
import { Dialog, DialogContent, DialogHeader, DialogTitle, DialogDescription } from "@/components/ui/dialog"
import {
  LogOut, Calendar, CheckCircle2, Image as ImageIcon,
  Search, Hash, FileText, ExternalLink, Settings, MessageSquarePlus, Download, ChevronRight, Send, ClipboardPlus
} from "lucide-react"
...
export default function PanelMedicoClient({ dentist, procedures }: { dentist: any; procedures?: any[] }) {
  ...
  const [derivacionData, setDerivacionData] = useState({
    patientApellido: "", patientNombre: "", patientDni: "", patientBirthDate: "",
    cobertura: "particular", obraSocial: "", nroAfiliado: "",
    procedures: "", indicaciones: ""
  })
  ...
  const handleSendDerivacion = async (e: React.FormEvent) => {
    e.preventDefault()
    if (!derivacionData.patientApellido || !derivacionData.patientNombre) return toast.error("Apellido y nombre del paciente son obligatorios")
    if (!derivacionData.procedures) return toast.error("Indicá al menos un estudio solicitado")
    setLoadingDerivacion(true)
    const res = await createDerivacion({ dentistId: dentist.id, ...derivacionData })
    if (res.success) {
      toast.success(`Derivación enviada. Código: ${res.code}`, { description: "Presentá este código en recepción." })
      setShowDerivacionModal(false)
      setDerivacionData({ patientApellido: "", patientNombre: "", patientDni: "", patientBirthDate: "", cobertura: "particular", obraSocial: "", nroAfiliado: "", procedures: "", indicaciones: "" })
    } else {
      toast.error(res.error || "Error al enviar la derivación")
    }
  ...
  return (
    ...
    >
      ...
      <Dialog open={showDerivacionModal} onOpenChange={setShowDerivacionModal}>
        <DialogContent className="sm:max-w-[560px] bg-white rounded-2xl border-t-8 border-red-600 p-8 outline-none">
          <DialogHeader>
            <DialogTitle className="text-2xl font-bold uppercase tracking-tight text-neutral-900 flex items-center gap-2">
              <ClipboardPlus className="text-red-600" size={24}/> Nueva Derivación
            </DialogTitle>
            <DialogDescription className="text-neutral-500">
              Completá los datos del paciente y los estudios solicitados. Al confirmar, recepción recibirá la solicitud.
            </DialogDescription>
          </DialogHeader>
          <form onSubmit={handleSendDerivacion} className="space-y-4 py-3">
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1">
                <Label className="text-xs font-bold uppercase text-neutral-500">Apellido *</Label>
                <Input value={derivacionData.patientApellido} onChange={e => setDerivacionData(d => ({...d, patientApellido: e.target.value}))} className="h-11 uppercase font-bold border-2" placeholder="Ej: GOMEZ" />
              </div>
              <div className="space-y-1">
                <Label className="text-xs font-bold uppercase text-neutral-500">Nombre *</Label>
                <Input value={derivacionData.patientNombre} onChange={e => setDerivacionData(d => ({...d, patientNombre: e.target.value}))} className="h-11 uppercase font-bold border-2" placeholder="Ej: MARIA" />
              </div>
              <div className="space-y-1">
                <Label className="text-xs font-bold uppercase text-neutral-500">DNI</Label>
                <Input value={derivacionData.patientDni} onChange={e => setDerivacionData(d => ({...d, patientDni: e.target.value}))} className="h-11 font-bold border-2" placeholder="Ej: 35123456" />
              </div>
              <div className="space-y-1">
                <Label className="text-xs font-bold uppercase text-neutral-500">Fecha Nac.</Label>
                <Input type="date" value={derivacionData.patientBirthDate} onChange={e => setDerivacionData(d => ({...d, patientBirthDate: e.target.value}))} className="h-11 font-bold border-2" />
              </div>
            </div>

            <div className="space-y-1">
              <Label className="text-xs font-bold uppercase text-neutral-500">Cobertura</Label>
              <Select value={derivacionData.cobertura} onValueChange={v => setDerivacionData(d => ({...d, cobertura: v}))}>
                <SelectTrigger className="h-11 font-bold border-2 bg-neutral-50"><SelectValue /></SelectTrigger>
                <SelectContent>
                  <SelectItem value="particular" className="font-medium">Particular</SelectItem>
                  <SelectItem value="obra_social" className="font-medium">Obra Social / Mutual</SelectItem>
                </SelectContent>
              </Select>
            </div>

            {derivacionData.cobertura === "obra_social" && (
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1">
                  <Label className="text-xs font-bold uppercase text-neutral-500">Obra Social</Label>
                  <Input value={derivacionData.obraSocial} onChange={e => setDerivacionData(d => ({...d, obraSocial: e.target.value}))} className="h-11 uppercase font-bold border-2" placeholder="Ej: OSDE, PAMI..." />
                </div>
                <div className="space-y-1">
                  <Label className="text-xs font-bold uppercase text-neutral-500">Nro. Afiliado</Label>
                  <Input value={derivacionData.nroAfiliado} onChange={e => setDerivacionData(d => ({...d, nroAfiliado: e.target.value}))} className="h-11 font-bold border-2" />
                </div>
              </div>
            )}

            <div className="space-y-1">
              <Label className="text-xs font-bold uppercase text-neutral-500">Estudios solicitados *</Label>
              <Input value={derivacionData.procedures} onChange={e => setDerivacionData(d => ({...d, procedures: e.target.value}))} className="h-11 font-bold border-2" placeholder="Ej: RX Periapical P11, P12 — Panorámica" />
            </div>

            <div className="space-y-1">
              <Label className="text-xs font-bold uppercase text-neutral-500">Indicaciones / Observaciones</Label>
              <textarea
                value={derivacionData.indicaciones}
                onChange={e => setDerivacionData(d => ({...d, indicaciones: e.target.value}))}
                placeholder="Indicaciones clínicas adicionales..."
                className="flex w-full rounded-xl border-2 border-slate-200 bg-neutral-50 px-4 py-3 text-sm font-medium placeholder:text-neutral-400 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-red-600 resize-none h-20"
              />
            </div>

            <Button type="submit" disabled={loadingDerivacion} className="w-full h-12 bg-red-600 hover:bg-red-700 text-white font-bold uppercase tracking-wider rounded-xl shadow-lg flex items-center justify-center gap-2">
              <Send size={16}/> {loadingDerivacion ? "Enviando..." : "Confirmar Derivación"}
            </Button>
          </form>
        </DialogContent>
      </Dialog>
      ...
      <div className="max-w-6xl mx-auto px-4 -mt-6 relative z-20 space-y-6">
        ...
        <div className="flex flex-col md:flex-row gap-4">
          ...
          <Button
            ...
            className="h-16 bg-red-600 hover:bg-red-700 text-white font-bold uppercase tracking-wider px-6 rounded-xl shadow-md flex items-center justify-center gap-2 shrink-0 transition-all"
          >
            <ClipboardPlus size={20} />
            <span>Nueva Derivación</span>
          </Button>

          <Button
            onClick={() => setShowTicketModal(true)}
            className="h-16 bg-neutral-900 hover:bg-neutral-800 text-white font-bold uppercase tracking-wider px-6 rounded-xl shadow-md flex items-center justify-center gap-2 shrink-0 border border-neutral-700 hover:border-neutral-500 transition-all"
          >
